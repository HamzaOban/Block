...
public class GameOverManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        puzzlePieces.Add(new int[,] { { 1 } }); 
        puzzlePieces.Add(new int[,] { { 1, 1 } }); 
        puzzlePieces.Add(new int[,] { { 1 } , { 1 } });
        ...
        foreach (var slot in slots)
        {
            ...
            if (IsGameOver(pieceInt))
            {
                ...
                for (int i = 0; i <= numberBool; i++)
                {
                    ...
                    Debug.Log(numbers.Count + " Toplam");
    ...
    private void Update()
    {
        if(PlayerPrefs.GetInt("Rewarded") == 1)
        {
            ...
            numberBool = 0;
            numbers.Clear();
            revivePanel.GetComponent<Revive>().elapsedTime = 10f;
            revivePanel.SetActive(true);
            StartCoroutine(Revive.Instance.ReviveCount());
